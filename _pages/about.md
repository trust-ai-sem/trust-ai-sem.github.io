...
title: Trustworthy AI Seminar Series
---

<br>This is the homepage of the **Trustworthy AI Seminar Series**, organized by [Eugene Bagdasarian](https://people.cs.umass.edu/~eugene/) and [Emiliano De Cristofaro](https://emilianodc.com).
...
- May 1, 2026 (12pm EST / 9am PST / 5pm GMT)  
...
[[Recording]](https://youtu.be/X5DKWo1edIc)<details><br>**Abstract:**  Large Language Models (LLMs) are increasingly being integrated into various applications. The functionalities of recent LLMs can be flexibly modulated via natural language prompts. This renders them susceptible to targeted adversarial prompting, e.g., Prompt Injection (PI) attacks enable attackers to override original instructions and employed controls. So far, it was assumed that the user is directly prompting the LLM. But, what if it is not the user prompting? In this talk, I will present our recent work in which we revealed new attack vectors using Indirect Prompt Injection. This enables adversaries to remotely (without a direct interface) exploit LLM-integrated applications by strategically injecting prompts into data likely to be retrieved. We showed how processing retrieved prompts can act as arbitrary code execution, manipulate the application’s functionality, and control how and if other APIs are called. Despite the increasing integration and reliance on LLMs, effective mitigations of these emerging threats are currently lacking. By raising awareness of these vulnerabilities, we aim to promote the safe and responsible deployment of these powerful models and the development of robust defenses that protect users and systems from potential attacks. <br><br>**Bio:**  Sahar Abdelnabi is a PhD student at the CISPA Helmholtz Center for Information Security. Her research interests lie in the broad intersection of machine learning, LLMs in particular, with security, online safety, and sociopolitical aspects. </details>
...
[[Recording]](https://youtu.be/akt88Vu-Ytk)<details><br>**Abstract:** Machine learning is a powerful tool to detect Android malware with high accuracy. However, it suffers from the problem of concept drift: benign and malicious behavior changes over time, and current machine learning models have difficulty keeping up with this change and rapidly become ineffective. Concept drift happens for many reasons. For example, malware authors may add malicious functionalities to evade detection, or create new types of malware that's never seen before, and benign apps release updates to utilize new features provided by the Android SDK. Our research finds that, after training an Android malware classifier on one year's worth of data, the F1 score quickly dropped from 0.99 to 0.76 after 6 months of deployment on new test samples. In this talk, I will present new methods to make machine learning for Android malware detection more effective. I will show how to make malware classifiers robust against concept drift. Since machine learning technique needs to be continuously deployed, we use active learning: we select new samples for analysts to label, and then add the labeled samples to the training set to retrain the classifier. Our key idea is, similarity-based uncertainty is more robust against concept drift. Therefore, we combine contrastive learning with active learning. We propose a new hierarchical contrastive learning scheme, and a new sample selection technique to continuously train the Android malware classifier. Our results show that given the same analyst labeling budget to retrain the classifier, we can reduces the false negative rate from 14% (for the best baseline) to 9%, while also reducing the false positive rate (from 0.86% to 0.48%); and to maintain a steady F1 score over time, we can achieve 8X reduction in labels indeed. <br><br>**Bio:**  Yizheng Chen is an Assistant Professor of Computer Science at University of Maryland. She works at the intersection of AI and security. Her research is focused on using AI to solve security problems, such as detecting malware, vulnerability, attacks and fraud. She is also interested in improving the robustness of these machine learning models. Previously, she received her Ph.D. in Computer Science from the Georgia Institute of Technology, and was a postdoc at University of California, Berkeley and Columbia University. Her work has received an ACM CCS Best Paper Award Runner-up and a Google ASPIRE Award. She is a recipient of the Anita Borg Memorial Scholarship.<br>Homepage: [https://surrealyz.github.io/](https://surrealyz.github.io/) </details>

 
<img src="../images/xudong.png" style="float:right;width:100px;height:100px;margin-top:00px">
- June 28, 2023  
**Xudong Pan (Fudan University)**  
The Security Pitfalls of DNN Watermarking Algorithms under Neural Obfuscation  
[[Recording]](https://youtu.be/_8tpY4VLSt4)<details><br>**Abstract:** Well-trained deep neural networks are becoming an indispensable part of the model owner’s intellectual property. To trace and verify the model ownership, recent years witness a number of digital watermarking algorithms devised for deep neural networks (DNN). However, we observe almost none of them indeed considered the unique characteristics of DNNs, which leave them with common security pitfalls. In this talk, I will introduce our discovery on the common vulnerability of white-box watermarking schemes for deep neural networks (DNN). Specifically, I will show how to exploit several neuron invariance properties in both the weight space (KDD’23) and the structural space (USENIX Security’23) to remove the embedded watermarks in deep neural networks. Our attacks reduced the watermark verification rate of nine state-of-the-art DNN watermarking schemes to almost random, and the attacker can do it almost for free (provably no utility loss on the original model, no attacker’s knowledge on the watermark, and almost no computation cost). <br><br>**Bio:** Xudong Pan obtained his Bsc and PhD degree from Fudan University in 2018 and 2023. This year he will join Fudan University as a tenure-track faculty. He works at the interdisciplinary research area between AI and Security. Specifically, he is interested in the following three security aspects of deep learning ecosystem: AI Supply Chain Security, Privacy Risks of Open AI Systems, and Copyright Protection for AI Models. In the past five years, he has published 25+ papers at top-tier conference/journals including TPAMI, TKDE, NeurIPS, ICML, IEEE S&P (Oakland) and USENIX Security as the primary researcher. He is a member of the [Whitzard AI Team](https://whitzard-ai.github.io,) (led by Prof. Mi Zhang) at System Software and Security Lab (led by Prof. Min Yang). Homepage: [https://ravensanstete.github.io](https://ravensanstete.github.io) </details>
